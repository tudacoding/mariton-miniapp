import { MaritonToken, Mint, MintPublic } from "../contract/token";
...
export function useMaritonToken() {
    ...
    const mrtAddress = Address.parse("EQBAK2GFaOix6p9rRP2URa2Uf8Th8XvzuymnFPPycyUAGvCp");
    const claimAddress = Address.parse("EQByc9YFB3yeNTcSPo2WF5gvMYDKLsz9Sr2PpkVsoYlz-gyl");
    ...
    return {
        ...
        mrtBalance: MRT,
        withdrawTon: async () => {
            return await claimContract?.send(sender, {
                value: toNano("0.005")
            }, "withdraw")
        },
        ...
        MintClose: async () => {
            return await tokenContract?.send(sender, {
                value: toNano("0.005")
            }, "Owner: MintClose")
        },
        mintMRT: async () => {
