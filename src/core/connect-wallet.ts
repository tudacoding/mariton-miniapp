...
export class ConnectWalletHtmlScene implements ConnectScene {
  private connectDiv: HTMLDivElement = document.getElementById(
    "connect"
  ) as HTMLDivElement;
...
export class ConnectWalletCanvasScene
  extends Phaser.Scene
  implements ConnectScene
{
  public static readonly sceneKey = "ConnectWalletCanvasScene";
  private button!: ConnectWalletButton;

  constructor(
    public readonly gameFi: GameFi,
    private params: ConnectWalletButtonParams
  ) {
    super({
      key: ConnectWalletCanvasScene.sceneKey,
      active: true,
      visible: true,
    });
  }
...
export async function createConnectUi(
  config: Config,
  uiType: "html" | "canvas"
): Promise<ConnectScene> {
  ...
  if (uiType === "html") {
    ...
    return new ConnectWalletCanvasScene(gameFi, {
      style: "light",
      onError: (error) => {
